LCA <- function(
    ...
    SP_THOLD = 96, #Thresolhd below which species assignments are ignored, and only higuer taxonomic assignments are considered
    SP_MIN_THOLD = 98.7
    ){
  ...
  f <- f %>%
    ...
    mutate(Species = if_else(!is.na(Max_pident) & Max_pident < SP_THOLD, "", Species))
