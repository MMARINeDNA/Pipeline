...
PROCESSED_LOCATION <- "/Users/rpk/Downloads/20260530_COI" 
...
TAXONKIT_PATH <- "/Users/rpk/taxonkit"  # Pedro's laptop
system2(TAXONKIT_PATH, args = shQuote("version"))

# Read in primer data
primer.data <- read.csv("primer.data.csv")
...
MAX_AMPLICON_LENGTH <- 350 #primer.data %>% filter(name == PRIMERNAME) %>% pull(max_amplicon_length)
MIN_AMPLICON_LENGTH <- 200 #primer.data %>% filter(name == PRIMERNAME) %>% pull(min_amplicon_length)
OVERLAP <- 20 #primer.data %>% filter(name == PRIMERNAME) %>% pull(overlap)
...
system2("cp", args = c(shQuote(here("pipeline_code_4.0.R")), shQuote(CODE_LOCATION)))  # copy this pipeline code
...
sample.names3 <- sapply(strsplit(basename(fnFs), "_"), `[`, 3)
sample.names4 <- sapply(strsplit(basename(fnFs), "_"), `[`, 4)
sample.names <- paste(sample.names1, sample.names2, sample.names3, sample.names4, sep = "_")
...
if (file.exists(paste0(DATABASE_LOCATION, PRIMERNAME, "_database.csv"))) {
  ...
  write.fasta(sequences = as.list(conv_table$Sequence[notseen]),
              ...
              file.out = paste0(PROCESSED_LOCATION, "/", RUN_NAME, "/outputs/seqs_to_annotate.fasta"))
} else {
  write.fasta(sequences = as.list(conv_table$Sequence),
              names = as.list(conv_table$Hash),
              file.out = paste0("/Users/rpk/Downloads/rockfishDIN/outputs/seqs_to_annotate.fasta"))
}
...
if (file.size(paste0(PROCESSED_LOCATION, "/", RUN_NAME, "_", PRIMERNAME, "/outputs/new_annotations.txt")) == 0L) {
  ...
  db <- LCA(BLASTOUTPUT = paste0("/Users/rpk/Downloads/rockfishDIN/outputs/new_annotations.txt"),
            FASTA = paste0("/Users/rpk/Downloads/rockfishDIN/outputs/seqs_to_annotate.fasta"),
            SP_THOLD = SP_THOLD)
...
write_csv(tax_table, file = paste0("/Users/rpk/Downloads/rockfishDIN/outputs/taxon_table.csv"))
...
write_csv(tax_wide, file = paste0("/Users/rpk/Downloads/rockfishDIN/outputs/taxon_table_wide.csv"))
